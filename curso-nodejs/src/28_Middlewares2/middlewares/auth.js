...
const userIsAdmin = role => {
	console.log('userIsAdmin -> role:', role)
	return role === 'admin'
}
...
const isAuth = (req, res, next) => {
	try {
		...
		if (token) {
			...
			console.log('isAuth() - req.body.userId:', req.body.userId)
			console.log('isAuth() - jwt.verify:', data)

			if (data.userId !== req.body.userId && !userIsAdmin(data.role)) {
				throw {
					code: 403,
